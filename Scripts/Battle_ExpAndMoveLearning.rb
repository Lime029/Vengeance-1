class PokeBattle_Battle
    ...
    def pbGainExpOne(idxParty,defeatedBattler,numPartic,expShare,expAll,showMessages=true)
      ...
      Achievements.incrementProgress("GAIN_EXPERIENCE",expGained)
      # "Exp gained" message
      if showMessages
        if isOutsider
      ...
      loop do
        ...
        if pbDisplayConfirm(_INTL("Forget a move to learn {1}?",moveName))
          ...
          if forgetMove>=0
            ...
            oldMoveID = pkmn.moves[forgetMove].id
            ...
            pbDisplay(_INTL("{1} learned {2}!",pkmnName,moveName)) { pbSEPlay("Pkmn move learnt") }
            # Replaces move memory move when replacing moves
            if pkmn.moveMemory.length>0
              for i in 0...pkmn.moveMemory.length
                if pkmn.moveMemory[i].id == oldMoveID
                  pkmn.moveMemory[i] = PokeBattle_Move.from_pokemon_move(self, Pokemon::Move.new(newMove))
                  echo("\n[AI - #{Time.now - $time}] #{pbThis}'s #{oldMoveName} was replaced with #{pkmn.moves[forgetMove].name} in its move memory.")
                  break
                end
              end
            end
