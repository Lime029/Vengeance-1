class PokeBattle_Scene
    ...
    def pbBattleIntroAnimation
      ...
      if @battle.showAnims
        for i in 0...@battle.sideSizes[1]
          ...
          Achievements.incrementProgress("FIND_SHINY",1)
        end
      end
    end
    ...
    def pbFindMoveAnimation(moveID, idxUser, hitNum)
      begin
        ...
        typeDefaultAnim = {
          :NORMAL   => [:TACKLE,:SONICBOOM,:LUCKYCHANT,:EXTREMESPEED,:SWIFT,:FLASH],
           :FIGHTING => [:COUNTER,:FOCUSBLAST,:BULKUP,:LOWSWEEP,nil,:HELPINGHAND],
           :FLYING   => [:ACROBATICS,:TWISTER,:ROOST,nil,:AIRCUTTER,:FEATHERDANCE],
           :POISON   => [:POISONJAB,:SLUDGE,:ACIDARMOR,:POISONJAB,:SLUDGEWAVE,:TOXIC],
           :GROUND   => [:DRILLRUN,:MUDSHOT,:SANDSTORM,:EARTHQUAKE,:EARTHPOWER,:SANDATTACK],
           :ROCK     => [:ROCKWRECKER,:ANCIENTPOWER,:ROCKPOLISH,:ROCKSLIDE,:ANCIENTPOWER,:STEALTHROCK],
           :BUG      => [:ATTACKORDER,:STRUGGLEBUG,:DEFENDORDER,:TWINEEDLE,:BUGBUZZ,:STRINGSHOT],
           :GHOST    => [:SHADOWSNEAK,:HEX,:GRUDGE,nil,:OMINOUSWIND,:SPITE],
           :STEEL    => [:IRONHEAD,:FLASHCANNON,:IRONDEFENSE,:METALBURST,:MIRRORSHOT,:METALSOUND],
           :FIRE     => [:FLAMECHARGE,:BLUEFLARE,:MORNINGSUN,:OUTRAGE,:OVERHEAT,:WILLOWISP],
           :WATER    => [:LIQUIDATION,:HYDROCANNON,:SPLASH,:WATERFALL,:SURF,:SOAK],
           :GRASS    => [:LEAFAGE,:LEAFSTORM,:SYNTHESIS,:PETALBLIZZARD,:PETALDANCE,:SLEEPPOWDER],
           :ELECTRIC => [:ZINGZAP,:THUNDERSHOCK,:CHARGE,:VOLTTACKLE,:DISCHARGE,:THUNDERWAVE],
           :PSYCHIC  => [:PSYCHOCUT,:EXTRASENSORY,:CALMMIND,nil,:PSYCHIC,:TELEKINESIS],
           :ICE      => [:ICEBALL,:ICEBEAM,:HAIL,:AVALANCHE,:SHEERCOLD,:FREEZEDRY],
           :DRAGON   => [:DRAGONRUSH,:DRAGONRAGE,:DRAGONDANCE,:DRAGONTAIL,:DRACOMETEOR,nil],
           :DARK     => [:PAYBACK,:DARKPULSE,:LUNARDANCE,:BRUTALSWING,:NIGHTDAZE,:MEMENTO],
           :FAIRY    => [:RETURN,:FAIRYWIND,:AROMATICMIST,nil,:DAZZLINGGLEAM,:LOVELYKISS]
        }
