...
class PokeBattle_Move_100 < PokeBattle_WeatherMove
  ...
  class PokeBattle_Move_109 < PokeBattle_Move
    def pbEffectGeneral(user)
      if user.pbOwnedByPlayer? && $fefieldeffect != 29
        @battle.field.effects[PBEffects::PayDay] += 5*user.level
        if [6,25,32].include?($fefieldeffect)
          @battle.field.effects[PBEffects::PayDay] += 95*user.level
        end
      end
      if $fefieldeffect == 25
        @battle.pbDisplay(_INTL("Gems were scattered everywhere!"))
      elsif $fefieldeffect == 32
        @battle.pbDisplay(_INTL("Treasure was scattered everywhere!"))
      else # Performance Stage message played during damage boost
        @battle.pbDisplay(_INTL("Coins were scattered everywhere!"))
      end
      if $fefieldeffect == 29
        @battle.pbDisplay(_INTL("The donation was graciously accepted!"))
        if user.canHeal?
          user.pbRecoverHP(user.totalhp/2)
  ...
  class PokeBattle_Move_10A < PokeBattle_Move
    def ignoresReflect?(user); return true; end
  ...
  class PokeBattle_Move_10B < PokeBattle_Move
    ...
    def pbCrashDamage(user)
      return if !user.takesIndirectDamage? || [8,12,19].include?($fefieldeffect)
      @battle.pbDisplay(_INTL("{1} kept going and crashed!",user.pbThis))
      @battle.scene.pbDamageAnimation(user)
      user.pbReduceHP(user.totalhp/2,false)
      if $fefieldeffect == 46
        @battle.pbDisplay(_INTL("{1} crashed through the ice and kept going!",user.pbThis))
        @battle.scene.pbDamageAnimation(user)
        user.pbReduceHP(user.totalhp/2,false)
      end
  ...
  class PokeBattle_Move_10C < PokeBattle_Move
    ...
    def pbOnStartUse(user,targets)
      ...
      @subLife = user.totalhp/3 if $fefieldeffect == 5
      user.pbItemHPHealCheck
      user.effects[PBEffects::UsingSubstitute] = true
    end
    ...
    def pbEffectGeneral(user)
      ...
      user.effects[PBEffects::UsingSubstitute] = false
    end
  end
  ...
  class PokeBattle_Move_10D < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      ...
      user.pbRaiseStatStage([:ATTACK,:DEFENSE],1,user,true)
    end
    ...
    def pbShowAnimation(id,user,targets,hitNum=0,showAnimation=true)
      if user.pbHasType?(:GHOST)
        ...
        @battle.pbCommonAnimation("CurseNoGhost",user)
      end
    end
  end
  ...
  class PokeBattle_Move_10E < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      target.eachMove do |m|
        ...
        if [39,40].include?($fefieldeffect)
          reduction = [6,m.pp].min
        else
  ...
  class PokeBattle_Move_10F < PokeBattle_Move
    def pbFailsAgainstTarget?(user,target)
      return false if [38,40,42].include?($fefieldeffect)
  ...
  class PokeBattle_Move_110 < PokeBattle_Move
    def pbEffectAfterAllHits(user,target)
      ...
      if user.pbOwnSide.effects[PBEffects::StealthRock]
        ...
        @battle.pbDisplay(_INTL("{1} spun away stealth rocks!",user.pbThis))
      end
      if user.pbOwnSide.effects[PBEffects::Spikes]>0
        user.pbOwnSide.effects[PBEffects::Spikes] = 0
        @battle.pbDisplay(_INTL("{1} spun away spikes!",user.pbThis))
      end
      if user.pbOwnSide.effects[PBEffects::ToxicSpikes]>0
        user.pbOwnSide.effects[PBEffects::ToxicSpikes] = 0
        @battle.pbDisplay(_INTL("{1} spun away poison spikes!",user.pbThis))
      end
      if user.pbOwnSide.effects[PBEffects::StickyWeb]
        user.pbOwnSide.effects[PBEffects::StickyWeb] = false
        user.pbOwnSide.effects[PBEffects::StickyWebUser] = -1
        @battle.pbDisplay(_INTL("{1} spun away sticky webs!",user.pbThis))
      end
  ...
  class PokeBattle_Move_111 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      if @battle.futureSight # Attack is hitting
        @battle.pbDisplay(_INTL("The future is now, thanks to science!")) if $fefieldeffect == 17 &&
                                                                             @id == :FUTURESIGHT
        return
      end
  ...
  class PokeBattle_Move_112 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      ...
      showAnim = true
      if [12,19].include?($fefieldeffect)
        increment = 2
      else
        increment = 1
      end
      if user.pbCanRaiseStatStage?(:DEFENSE,user,self)
        if user.pbRaiseStatStage(:DEFENSE,increment,user,showAnim)
          user.effects[PBEffects::StockpileDef] += 1
          showAnim = false
        end
      end
      if user.pbCanRaiseStatStage?(:SPECIAL_DEFENSE,user,self)
        if user.pbRaiseStatStage(:SPECIAL_DEFENSE,increment,user,showAnim)
  ...
  class PokeBattle_Move_114 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      ...
      if $fefieldeffect == 19 && user.effects[PBEffects::Stockpile] == 3
        user.pbCureStatus
      elsif $fefieldeffect == 26 && user.grounded? && user.pbCanPoison?(user,false,self)
        user.pbPoison
      end
      @battle.pbDisplay(_INTL("{1}'s stockpiled effect wore off!",user.pbThis))
      showAnim = true
  ...
  class PokeBattle_Move_115 < PokeBattle_Move
    ...
    def pbMoveFailed?(user,targets)
      if user.effects[PBEffects::FocusPunch] && user.lastHPLost>0 && ![20,48].include?($fefieldeffect) ||
         [1,18].include?($fefieldeffect)
  ...
  class PokeBattle_Move_117 < PokeBattle_Move
    def pbEffectGeneral(user)
      ...
      elsif @id == :RAGEPOWDER && ($fefieldeffect == 33 && $fecounter >= 3 || $fefieldeffect == 39)
  ...
  class PokeBattle_Move_118 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      if $fefieldeffect == 37
        ...
        @battle.field.effects[PBEffects::Gravity] = 3 + @battle.pbRandom(6)
      else
  ...
  class PokeBattle_Move_119 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      if [1,17,18].include?($fefieldeffect)
        user.effects[PBEffects::MagnetRise] = 8
      else
  ...
  class PokeBattle_Move_11C < PokeBattle_Move
    ...
    def pbCalcTypeModSingle(moveTypes,defType,user,target)
      return Effectiveness::NORMAL_EFFECTIVE_ONE if moveTypes.include?(:GROUND) && defType == :FLYING
      return super
    end
  ...
  class PokeBattle_Move_11F < PokeBattle_Move
    def pbEffectGeneral(user)
      if @battle.field.effects[PBEffects::TrickRoom]>0
        ...
        if [5,25,30,35,37,42].include?($fefieldeffect)
          @battle.field.effects[PBEffects::TrickRoom] = 8
        elsif $fefieldeffect == 38
          @battle.field.effects[PBEffects::TrickRoom] = 3 + @battle.pbRandom(6)
        else
  ...
  class PokeBattle_Move_120 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      ...
      if @battle.pbSwapBattlers(idxA,idxB)
        ...
        if [30,40].include?($fefieldeffect)
          user.pbRaiseStatStage(:EVASION,1,user)
          @battle.battlers[idxB].pbRaiseStatStage(:EVASION,1,user)
        end
      end
    end
  end
  ...
  class PokeBattle_Move_122 < PokeBattle_Move
    def pbGetDefenseStats(user,target)
      ...
      statStage = target.stages[:DEFENSE]+6
      if $fefieldeffect == 36 && (user.hasActiveAbility?(:PALINDROME) || target.hasActiveAbility?(:PALINDROME))
        stageValue = 12 - stageValue
      end
      return statValue, statStage
  ...
  class PokeBattle_Move_123 < PokeBattle_Move
    def pbFailsAgainstTarget?(user,target)
      return false if $fefieldeffect == 37
    ...
    def pbEffectGeneral(user)
      if @battle.field.effects[PBEffects::WonderRoom]>0
        ...
        if [4,25,35,37,42].include?($fefieldeffect)
          ...
          @battle.field.effects[PBEffects::WonderRoom] = 3 + @battle.pbRandom(6)
        else
  ...
  class PokeBattle_Move_133 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      if [6,31].include?($fefieldeffect) && !target.fainted? && !target.effects[PBEffects::HelpingHand] &&
         !pbMoveFailedTargetAlreadyMoved?(target)
  ...
  class PokeBattle_Move_135 < PokeBattle_FreezeMove
    def pbCalcTypeModSingle(moveTypes,defType,user,target)
      ...
      return 2*super if defType == :WATER
      return super
    end
  end
  ...
  #===============================================================================
  # Increases the user's Defense by 2 stages once per use. (Diamond Storm)
  #===============================================================================
  class PokeBattle_Move_136 < PokeBattle_Move_02F
    # Empty to not trigger regular additional effect
    def pbAdditionalEffect(user,target)
    end
    
    def pbEffectsAfterMove(user,targets,move,numHits)
      return if user.hasActiveAbility?(:SHEERFORCE)
      chance = pbAdditionalEffectChance(user)
  ...
  class PokeBattle_Move_137 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      if [17,18].include?($fefieldeffect)
        target.pbRaiseStatStage([:DEFENSE,:SPECIAL_DEFENSE],2,user)
      else
        target.pbRaiseStatStage([:DEFENSE,:SPECIAL_DEFENSE],1,user)
      end
    end
  ...
  class PokeBattle_Move_138 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      if [3,9].include?($fefieldeffect) || $fefieldeffect == 33 && $fecounter >= 3
        target.pbRaiseStatStage(:SPECIAL_DEFENSE,2,user)
      else
  ...
  # (Noble Roar, Tearful Look)
  #===============================================================================
  class PokeBattle_Move_13A < PokeBattle_TargetStatDownMove
    def ignoresSubstitute?(user); return true; end
  
    def getStatDown
      statDown = [:ATTACK,1,:SPECIAL_ATTACK,1]
      if [5,6,29,31,32].include?($fefieldeffect) && @id == :NOBLEROAR
        statDown = [:ATTACK,2,:SPECIAL_ATTACK,2]
      elsif $fefieldeffect == 40 && @id == :TEARFULLOOK
        statDown = [:ATTACK,2,:SPECIAL_ATTACK,2]
      end
      return statDown
    end
  ...
  class PokeBattle_Move_13C < PokeBattle_TargetStatDownMove
    ...
    def pbEffectGeneral(user)
      super
      return if damagingMove? || $fefieldeffect != 29
      user.pbRaiseStatStage(:SPECIAL_ATTACK,1,user)
    end
  end
  ...
  class PokeBattle_Move_13D < PokeBattle_TargetStatDownMove
    def getStatDown
      ...
      if [1,18,40,42].include?($fefieldeffect)
  ...
  class PokeBattle_Move_13E < PokeBattle_Move
    def pbMoveFailed?(user,targets)
      ...
      @battle.eachBattler do |b|
        ...
        if $fefieldeffect == 33 && user.index == b.index
    ...
    def pbEffectAgainstTarget(user,target)
      if $fefieldeffect == 2
        target.pbRaiseStatStage([:ATTACK,:SPECIAL_ATTACK,:SPECIAL_DEFENSE],2,user)
      elsif [15,33].include?($fefieldeffect)
        target.pbRaiseStatStage([:ATTACK,:SPECIAL_ATTACK],2,user)
      else
        target.pbRaiseStatStage([:ATTACK,:SPECIAL_ATTACK],1,user)
      end
    end
  end
    ...
    def pbEffectAgainstTarget(user,target)
      if $fefieldeffect == 2
        ...
        when 2,3
          ...
          target.pbRaiseStatStage([:DEFENSE,:SPECIAL_DEFENSE],3,user)
        end
      else
  ...
  class PokeBattle_Move_140 < PokeBattle_Move
    def pbMoveFailed?(user,targets)
      ...
      targets.each do |b|
        next if !b || b.fainted?
        next if !b.poisoned? && ![10,11,19,26,41].include?($fefieldeffect)
  ...
  class PokeBattle_Move_141 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      ...
      if !user.hasActiveItem?(:EVERSTONE) && ![35,36].include?($fefieldeffect)
        @battle.changeField(36,"The terrain was inverted!",3,user.hasTerrainExtender?,true)
      end
    end
  end
  ...
  class PokeBattle_Move_143 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      ...
      @battle.pbDisplay(_INTL("{1} transformed into the {2} type!",target.pbThis,typeName))
      if [31,42].include?($fefieldeffect) && !target.effects[PBEffects::Curse]
  ...
  class PokeBattle_Move_148 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      ...
      @battle.pbDisplay(_INTL("{1} is covered in powder!",target.pbThis))
    end
  end
  ...
  #===============================================================================
  # This round, the user's side is unaffected by damaging moves. Fails if it's not
  # the user's first turn in battle. (Mat Block)
  #===============================================================================
  ...
  # the user of a stopped contact move by 1 stage. (King's Shield)
  #===============================================================================
  ...
  class PokeBattle_Move_14E < PokeBattle_TwoTurnMove
    ...
    def pbEffectGeneral(user)
      ...
      user.pbRaiseStatStage([:SPECIAL_ATTACK,:SPECIAL_DEFENSE,:SPEED],2,user)
    end
    
    def pbShowAnimation(id,user,targets,hitNum=0,showAnimation=true)
      if @chargingTurn# && !@damagingTurn # Charging anim
        @battle.pbCommonAnimation("Geomancy charging",user)
        @chargingTurn = false
      else
        super
      end
  ...
  class PokeBattle_Move_14F < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      ...
      if $fefieldeffect == 31 && @id == :DRAININGKISS && target.asleep?
        target.pbCureStatus
      end
    end
  end
  ...
  #===============================================================================
  class PokeBattle_Move_151 < PokeBattle_TargetStatDownMove
    def getStatDown
      statDown = [:ATTACK,1,:SPECIAL_ATTACK,1]
      if $fefieldeffect == 39
  ...
  class PokeBattle_Move_154 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      @battle.changeField(1,"Electricity ran across the battlefield!",5,user.hasTerrainExtender?,true)
    end
  ...
  class PokeBattle_Move_155 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      @battle.changeField(2,"Grass grew over the battlefield!",5,user.hasTerrainExtender?,true)
    end
  ...
  class PokeBattle_Move_156 < PokeBattle_Move
    ...
    def pbEffectGeneral(user)
      @battle.changeField(3,"Mist swirled about the battlefield!",5,user.hasTerrainExtender?,true)
    end
  ...
  class PokeBattle_Move_159 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      if target.pbCanPoison?(user,false,self)
        if [11,41].include?($fefieldeffect)
  ...
  class PokeBattle_Move_15B < PokeBattle_HealingMove
    def pbFailsAgainstTarget?(user,target)
      if target.status == :NONE && !($fefieldeffect == 29 && user.canHeal?)
        @battle.pbDisplay(_INTL("But it failed!"))
        return true
      end
      return false
  ...
  class PokeBattle_Move_15D < PokeBattle_Move
    ...
    def pbCalcDamage(user,target,numTargets=1)
      if target.hasRaisedStatStages?
        @battle.pbCommonAnimation("StatDown",target)   # Stat stage-draining animation
  ...
  class PokeBattle_Move_15E < PokeBattle_Move
    def pbEffectGeneral(user)
      if $fefieldeffect == 17
        user.effects[PBEffects::LaserFocus] = 3
      else
  ...
  class PokeBattle_Move_15F < PokeBattle_StatDownMove
    def getStatDown
      statDown = []
      statDown = [:DEFENSE,1] if $fefieldeffect != 32 && @id == :CLANGINGSCALES
      return statDown
    end
  end
  ...
  class PokeBattle_Move_160 < PokeBattle_Move
    ...
    def pbEffectAgainstTarget(user,target)
      ...
      if target.pbCanLowerStatStage?(:ATTACK,user,self)
        if $fefieldeffect == 12
          target.pbLowerStatStage(:ATTACK,2,user)
        elsif $fefieldeffect == 42
          target.pbLowerStatStage([:ATTACK,:SPECIAL_ATTACK],1,user)
        else
          target.pbLowerStatStage(:ATTACK,1,user)
        end
      end
      healAmt = (healAmt*1.3).floor if [8,12,15,19,42,47].include?($fefieldeffect) ||
                                       $fefieldeffect == 33 && $fecounter >= 1
      # Heal user
      user.pbRecoverHPFromDrain(healAmt,target)
    end
  end
  ...
  #===============================================================================
  # User loses its Fire-type. Fails if user is not Fire-type. (Burn Up)
  #===============================================================================
  ...
  class PokeBattle_Move_167 < PokeBattle_Move
    def pbMoveFailed?(user,targets)
      if @battle.pbWeather != :Hail && ![9,13,25,28,30,34,39,46].include?($fefieldeffect)
        @battle.pbDisplay(_INTL("But it failed!"))
        return true
      end
    ...
    def pbEffectGeneral(user)
      ...
      user.pbOwnSide.effects[PBEffects::AuroraVeil] = (user.pbOwnSide.effects[PBEffects::AuroraVeil] * 1.5).ceil if user.hasActiveItem?(:LIGHTCLAY)
      user.pbOwnSide.effects[PBEffects::AuroraVeil] = (user.pbOwnSide.effects[PBEffects::AuroraVeil] * 1.5).ceil if [30,46].include?($fefieldeffect)
  ...
  =begin
    def pbOnStartUse(user,targets)
      ...
      if [12,20,48].include?($fefieldeffect)
        points += 1
      end
      if points <= 0
        @healAmount = 1
  ...
  =end
    ...
    def pbEffectGeneral(user)
      super
      return if ![21,26].include?($fefieldeffect) && !user.hasActiveAbility?(:WATERCOMPACTION)
      user.pbRaiseStatStageByAbility(:DEFENSE,2,user)
    end
  end
  ...
  class PokeBattle_Move_16E < PokeBattle_Move
    ...
    def pbHealAmount(user,target)
      ...
      elsif [2,31,48].include?($fefieldeffect) || $fefieldeffect == 33 && $fecounter >= 2
        healAmount = (target.totalhp*2/3.0).round
      else
        healAmount = (target.totalhp/2.0).round
      end
      return healAmount
    end
    ...
    def pbEffectAgainstTarget(user,target)
      ...
      if $fefieldeffect == 11 && target.pbCanPoison?(user,false,self)
  ...
  class PokeBattle_Move_170 < PokeBattle_Move
    ...
    def pbMoveFailed?(user,targets)
      if !@battle.moldBreaker && @id == :MINDBLOWN
  ...
  class PokeBattle_Move_171 < PokeBattle_Move
    def pbDisplayChargeMessage(user)
      ...
      @battle.pbDisplay(_INTL("{1} primed a shell trap!",user.pbThis))
    end
  ...
  #===============================================================================
  class PokeBattle_Move_176 < PokeBattle_Move_01F # Speed Up
  ...
  class PokeBattle_Move_177 < PokeBattle_Move
    def pbGetAttackStats(user,target)
      ...
      statStage = user.stages[:DEFENSE]+6
      if $fefieldeffect == 36 && (user.hasActiveAbility?(:PALINDROME) || target.hasActiveAbility?(:PALINDROME))
        stageValue = 12 - stageValue
      end
      return statValue, statStage
  ...
  #===============================================================================
  class PokeBattle_Move_179 < PokeBattle_StatUpMove
    def getStatUp
  ...
  # the target. (Decorate)
  #===============================================================================
  class PokeBattle_Move_17B < PokeBattle_TargetStatUpMove
    def getStatUp
      statUp = [:ATTACK,2,:SPECIAL_ATTACK,2]
      if $fefieldeffect == 29 || $fefieldeffect == 33 && $fecounter >= 1
  ...
  # In Doubles, not affected by Wide Guard.
  # (Dragon Darts)
  #===============================================================================
  ...
  class PokeBattle_Move_17E < PokeBattle_Move
    ...
    def pbHealAmount(user,target)
      if [3,9,21,29,31,43].include?($fefieldeffect)
  ...
  #===============================================================================
  class PokeBattle_Move_17F < PokeBattle_StatUpMove
    def getStatUp
      statUp = [:ATTACK,1,:DEFENSE,1,:SPECIAL_ATTACK,1,:SPECIAL_DEFENSE,1,:SPEED,1]
      if [5,31].include?($fefieldeffect)
        statUp = [:ATTACK,2,:DEFENSE,2,:SPECIAL_ATTACK,2,:SPECIAL_DEFENSE,2,:SPEED,2]
      end
      return statUp
    end
