module PokeBattle_BattleCommon
  ...
  =end
      if $Trainer.party_full?
        ...
        if pbConfirmMessage(_INTL("Would you like to keep {1} in your party?",pkmn.name))
          ...
          if cancel >= 0
            ...
            @initialItems[0][pbGet(1)] = pkmn2.item_id if @initialItems
        ...
        if swap
          ...
          @initialItems[0][pbPlayer.party.length-1] = pkmn2.item_id if @initialItems
        end
      else
    ...
    def pbThrowPokeBall(idxBattler,ball,catch_rate=nil,showPlayer=false)
      ...
      if $game_switches[72] # Can't Catch Wild Pokemon
        @scene.pbThrowAndDeflect(ball,1)
        pbDisplay(_INTL("An external signal seems to be preventing the Pokemon from being caught!\nMaybe catching it isn't possible for now..."))
        return
      end
