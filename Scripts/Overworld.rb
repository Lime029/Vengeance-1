...
Events.onMapUpdate += proc { |_sender,_e|
  ...
  def pbOnStepTaken(eventTriggered)
    ...
    if $PokemonGlobal.lure.nil?
      $PokemonGlobal.lure = 0
    end
    lure_active = ($PokemonGlobal.lure > 0)
    Events.onStepTaken.trigger(nil)
  #  Events.onStepTakenFieldMovement.trigger(nil,$game_player)
    handled = [nil]
    Events.onStepTakenTransferPossible.trigger(nil,handled)
    return if handled[0]
    pbBattleOnStepTaken(repel_active,lure_active) if !eventTriggered && !$game_temp.in_menu
  ...
  Events.onChangeDirection += proc {
    ...
    lure_active = ($PokemonGlobal.lure > 0)
    pbBattleOnStepTaken(repel_active,lure_active) if !$game_temp.in_menu
  }
  
  def pbBattleOnStepTaken(repel_active,lure_active)
    return if $Trainer.able_pokemon_count == 0
    return if !$PokemonEncounters.encounter_possible_here?
    encounter_type = $PokemonEncounters.encounter_type
    return if !encounter_type
    return if !$PokemonEncounters.encounter_triggered?(encounter_type,repel_active,true,lure_active)
    $PokemonTemp.encounterType = encounter_type
    encounter = $PokemonEncounters.choose_wild_pokemon(encounter_type,1,lure_active)
    encounter = EncounterModifier.trigger(encounter)
    if $PokemonEncounters.allow_encounter?(encounter, repel_active)
      if $PokemonEncounters.have_double_wild_battle?
        encounter2 = $PokemonEncounters.choose_wild_pokemon(encounter_type,1,lure_active)
  ...
  def pbSetCelebiRescue(x=$game_player.x,y=$game_player.y,map_id=$game_map.map_id,direction=$game_player.direction)
    $PokemonGlobal.celebiRescueMapId     = map_id
    $PokemonGlobal.celebiRescueX         = x
    $PokemonGlobal.celebiRescueY         = y
    $PokemonGlobal.celebiRescueDirection = direction
  end
  ...
  def pbItemBall(item,quantity=1)
    ...
    if $game_switches[118] && !item.is_key_item? # Random Items
      if item.is_machine?
        1000.times do
