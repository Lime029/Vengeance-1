...
class Pokemon
    ...
    # @return [Integer] the recoil damage taken by the Pokémon before fainting in battle
    attr_accessor :recoil_damage
    # @return [Integer] the number of times Barb Barrage has been used by the Pokémon
    attr_accessor :barb_barrage_uses
    # @return [Integer] the number of times Psyshield Bash has been used by the Pokémon
    attr_accessor :psyshield_bash_uses
    # @return [Integer] the form number with which the Pokemon started out
    attr_accessor :startForm
    # @return [Array<PokeBattle_Move>] the moves this Pokemon has revealed in battle
    attr_accessor :moveMemory
    ...
    def setRandomForm(allowSpecial=true)
      self.form_simple=(rand(getNumForms+1)) # must be no space next to "="
      if !allowSpecial
        if mega?
          self.form_simple=(getUnmegaForm)
        elsif [:CASTFORM,:GROUDON,:KYOGRE,:CHERRIM,:ARCEUS,:MELOETTA,:GENESECT,:GRENINJA,
                :AEGISLASH,:WISHIWASHI,:SILVALLY,:MIMIKYU,:CRAMORANT,:EISCUE,:MORPEKO].include?(@species)
          self.form_simple=0
        elsif @species == :DARMANITAN
          if form % 2 == 1 # Zen Mode
            self.form_simple=(form-1)
          end
        elsif @species == :ZYGARDE && form == 2 # Complete
          self.form_simple=(rand(2))
        elsif @species == :NECROZMA && form == 3 # Ultra
          self.form_simple=(rand(3))
        end
      end
    end
    
    # Returns number of ALTERNATE forms, not including base form
    def getNumForms
      return (GameData::Species::DATA.values.count { |sp| sp.species == @species })/2 - 1
    end
    ...
    def getAbilityList(sp_data=species_data)
      ret = []
      #sp_data = species_data
    ...
    def getMoveList
      ret = species_data.moves
      if $game_switches[117] # Random Moves/Abilities
        for m in species_data.moves
          1000.times do
            move_keys = GameData::Move::DATA.keys
            move_id = move_keys[rand(move_keys.length)]
            move_data = GameData::Move.get(move_id)
            if move_data.powerMove? || [:STRUGGLE,:HIDDENPOWERBUG,:HIDDENPOWERDAR,
               :HIDDENPOWERDRA,:HIDDENPOWERELE,:HIDDENPOWERFAI,:HIDDENPOWERFIG,
               :HIDDENPOWERFIR,:HIDDENPOWERFLY,:HIDDENPOWERGHO,:HIDDENPOWERGRA,
               :HIDDENPOWERGRO,:HIDDENPOWERICE,:HIDDENPOWERPOI,:HIDDENPOWERPSY,
               :HIDDENPOWERROC,:HIDDENPOWERSTE,:HIDDENPOWERWAT].include?(move_data.id)
              next
            end
            m[1] = move_data
            break
    ...
    def wild_reset_moves
      ...
      if $game_switches[117] # Random Moves/Abilities
        ...
        ability_id = ability_keys[rand(ability_keys.length)]
        if ![:ASONE,:ABILITY].include?(ability_id) # Keep original
          self.ability=GameData::Ability.get(ability_id)
        end
      end
      moveset.each { |m| 
        next if m[0] > this_level
        knowable_moves.push(m[1])
      }
      knowable_moves.push(GameData::Move.get(:SPLASH)) if knowable_moves.length == 0
      # Remove duplicates (retaining the latest copy of each move)
      knowable_moves = knowable_moves.reverse
      knowable_moves |= []
      knowable_moves = knowable_moves.reverse
      ...
      j=1
      threshold=MAX_MOVES
      if knowable_moves.length<MAX_MOVES
        threshold=knowable_moves.length
      end
      leftover=MAX_MOVES-threshold
      damagemoves=[]
      damagemoveindexes=[]
      k=0
      for i in knowable_moves
        temp=Pokemon::Move.new(i)
        if temp.base_damage>0
      ...
      if damagemoves.length!=0
        ...
        @moves[0]=damagemoves[r] # Guarantees 1st move as STAB
        knowable_moves.delete_at(damagemoveindexes[r]) # Removes selected move from move list
      else
        j=0
      end
      while j<threshold
        r=rand(knowable_moves.length)
        @moves[j]=Pokemon::Move.new(knowable_moves[r])
        knowable_moves.delete_at(r)
        j+=1
      end
  =begin
      while leftover > 0
        @moves[4-leftover]=Pokemon::Move.new(0)
        leftover-=1
      end
  =end
    end
    
    def trainer_reset_moves(bestSelection=false,testMon=false)
      this_level = self.level
      # Find all level-up moves that self could have learned
      moveset = self.getMoveList
      knowable_moves = []
      ...
      if bestSelection
        ...
        eggChance=$Trainer.badge_count*10 # Treats >100 as 100
        tmChance=($Trainer.badge_count-2)*5 # 5 when 3, increases 5 each badge (max 80)
      end
      # Adds TM/Tutor Moves
      tMoves = species_data.tutor_moves
      for m in tMoves
        next if rand(100)+1 > tmChance
        if $game_switches[117] # Random Moves/Abilities
          1000.times do
            move_keys = GameData::Move::DATA.keys
            move_id = move_keys[rand(move_keys.length)]
            move_data = GameData::Move.get(move_id)
            next if move_data.powerMove? || [:STRUGGLE,:HIDDENPOWERBUG,:HIDDENPOWERDAR,
                    :HIDDENPOWERDRA,:HIDDENPOWERELE,:HIDDENPOWERFAI,:HIDDENPOWERFIG,
                    :HIDDENPOWERFIR,:HIDDENPOWERFLY,:HIDDENPOWERGHO,:HIDDENPOWERGRA,
                    :HIDDENPOWERGRO,:HIDDENPOWERICE,:HIDDENPOWERPOI,:HIDDENPOWERPSY,
                    :HIDDENPOWERROC,:HIDDENPOWERSTE,:HIDDENPOWERWAT].include?(move_data.id)
            knowable_moves.push(move_data)
            break
          end
        else
          knowable_moves.push(m)
        end
      end
      # Adds Egg Moves
      eMoves = species_data.egg_moves
      for m in eMoves
        next if rand(100)+1 > eggChance
        if $game_switches[117] # Random Moves/Abilities
          1000.times do
            move_keys = GameData::Move::DATA.keys
            move_id = move_keys[rand(move_keys.length)]
      ...
      for i in knowable_moves
        temp=Pokemon::Move.new(i)
        if self.hasType?(temp.type) && temp.base_damage>0
      ...
      if testMon
        @moves[j]=Pokemon::Move.new(:METRONOME)
        j+=1
      end
      if stabmoves.length!=0
        r=rand(stabmoves.length)
        @moves[j]=stabmoves[r] # Guarantees 1 move as STAB
        knowable_moves.delete_at(stabmoveindexes[r]) # Removes selected move from move list
        j+=1
      end
      while j<threshold
        r=rand(knowable_moves.length)
  ...
  =end
    ...
    # Silently learns the given move. Will erase the second known move if it has to.
    ...
    def learn_move(move_id)
      ...
      # Delete the second known move if self now knows more moves than it should
      @moves.delete_at(1) if numMoves > MAX_MOVES
    end
    ...
    def compatible_with_move?(move_id)
      ...
      return move_data && (species_data.tutor_moves.include?(move_data.id) || 
             species_data.moves.include?(move_data.id) || species_data.egg_moves.include?(move_data.id))
    end
    ...
    def check_evolution_on_use_item(item_used)
      return check_evolution_on_trade(self) if item_used==:LINKINGCORD
      return check_evolution_internal { |pkmn, new_species, method, parameter|
