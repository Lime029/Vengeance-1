class PokeBattle_Battle
  ...
  =end
      ...
      if battler.effects[PBEffects::Trapping]>0 || battler.effects[PBEffects::MeanLook]>=0 ||
         battler.effects[PBEffects::Ingrain] && ![12,49].include?($fefieldeffect) || 
         battler.effects[PBEffects::NoRetreat] || @field.effects[PBEffects::FairyLock]>0
        partyScene.pbDisplay(_INTL("{1} can't be switched out!",battler.pbThis)) if partyScene
        return false
      end
      ...
      eachOtherSideBattler(idxBattler) do |b|
        ...
        if trappingTargetAbility?(battler,b)
          partyScene.pbDisplay(_INTL("{1}'s {2} prevents switching!",b.pbThis,b.abilityName)) if partyScene
          return false
        end
      end
      eachOtherSideBattler(idxBattler) do |b|
          ...
          if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
            ...
            if @internalBattle && @switchStyle && trainerBattle? && pbSideSize(0)==1 &&
              ...
              if enemyParty[idxPartyNew].hasAbility?(:ILLUSION)
                ...
                pokeName = enemyParty[idxPartyForName].name
              else
                pokeName = enemyParty[idxPartyForName].name
                if enemyParty[idxPartyForName].species_data.form_name != ""
                  pokeName+=" (#{enemyParty[idxPartyForName].species_data.form_name})"
                end
              end
              if pbDisplayConfirm(_INTL("{1} is about to send in {2}. Will you switch your Pokémon?",
                 opponent.full_name,pokeName))
    ...
    def pbMessagesOnReplace(idxBattler,idxParty)
      ...
      if party[idxParty].hasAbility?(:ILLUSION)
        new_index = pbLastInTeam(idxBattler)
    ...
    def pbOnActiveOne(battler)
      ...
      #---------------------------------------------------------------------------
      # ZUD - Z-Parting Shot/Z-Memento
      #---------------------------------------------------------------------------
      _ZUD_OnActiveEffects(battler) if defined?(Settings::ZUD_COMPAT)
      #---------------------------------------------------------------------------
      # Entry hazards
      if battler.affectedByEntryHazards?
        # Stealth Rock
        if battler.pbOwnSide.effects[PBEffects::StealthRock] && battler.takesIndirectDamage?
          if $fefieldeffect == 7
            type = [:ROCK,:FIRE]
          elsif $fefieldeffect == 13
            type = [:ROCK,:ICE]
          elsif $fefieldeffect == 25
            type = [:ROCK,crystalType]
          elsif $fefieldeffect == 28
            type = [:ROCK,:ICE]
          ...
          battler.pbInflictTypeScalingFixedDamage(type,battler.totalhp/quotient,_INTL("Pointed stones dug into {1}!",battler.pbThis))
        end
        # Spikes
        if battler.pbOwnSide.effects[PBEffects::Spikes]>0 && battler.takesIndirectDamage? &&
          ...
          spikesDiv *= 2 if [2,12].include?($fefieldeffect) # Halves damage
          spikesDiv /= 2 if $fefieldeffect == 14 # Doubles damage
          oldHP = battler.hp
          ...
          if $fefieldeffect == 7 && battler.pbCanBurn?(nil,false)
            battler.pbBurn(nil,_INTL("Their magma coating burned {1}!"),battler.pbThis(true))
          end
        end
        # Toxic Spikes
        if battler.pbOwnSide.effects[PBEffects::ToxicSpikes]>0 && !battler.airborne?
          if battler.pbHasType?(:POISON) && $fefieldeffect != 10
        ...
        # Sticky Web
        if battler.pbOwnSide.effects[PBEffects::StickyWeb] && !battler.airborne?
          ...
          if battler.pbCanLowerStatStage?(:SPEED)
            ...
            if [15,47].include?($fefieldeffect)
              battler.pbLowerStatStage(:SPEED,2,stickyuser)
            else
        ...
        # Corrosion Field Damage
        if $fefieldeffect == 10 && battler.takesCorrosiveFieldDamage?
          battler.pbInflictTypeScalingFixedDamage(:POISON,battler.totalhp/4,_INTL("{1} was seared by the corrosion!",battler.pbThis))
        end
      end
    ...
    def pbPriorityNeutralizingGas
      eachBattler do |b|
        ...
        if b.hasActiveAbility?(:NEUTRALIZINGGAS,false,true) && !b.effects[PBEffects::GastroAcid] &&
          ...
          pbDisplay(_INTL("{1}'s gas nullified all abilities!",b.pbThis))
          @field.effects[PBEffects::NeutralizingGas] = true
          return
        end
      end
    end
    ...
    def pbActivateHealingWish(battler)
      ...
      if @positions[battler.index].effects[PBEffects::HealingWish]
        ...
        if [31,34].include?($fefieldeffect)
          battler.pbRaiseStatStage([:ATTACK,:SPECIAL_ATTACK],1)
        end
      ...
      if @positions[battler.index].effects[PBEffects::LunarDance]
        ...
        if $fefieldeffect == 34
          ...
          battler.pbRaiseStatStage([:ATTACK,:DEFENSE,:SPECIAL_ATTACK,:SPECIAL_DEFENSE,
                                    :SPEED,:ACCURACY,:EVASION],1)
        end
