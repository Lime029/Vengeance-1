class PokeBattle_Battle
    ...
    def pbCommandPhase
      ...
      @battleAI.setScores # Set all scores for the round at once and then choose later on
      # Choose actions for the round (player first, then AI)
      pbCommandPhaseLoop(true)    # Player chooses its actions
      return if @decision!=0   # Battle ended, stop choosing actions
      pbCommandPhaseLoop(false)   # AI chooses its actions
      for b in @battlers
        next if !b # Potentially the case with uneven sides
        b.moveChoice = nil
        b.switchChoice = nil
        b.itemChoice = nil
      end
