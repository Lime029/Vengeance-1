module GameData
    class Trainer
      ...
      def to_trainer
        ...
        pkmnList = @pokemon.clone
        if $game_variables[55] > 0 # Team size cutting for PWT
          (pkmnList.length - $game_variables[55]).times do
            pkmnList.delete_at(rand(pkmnList.length))
          end
        end
        pkmnList.each do |pkmn_data|
          species = GameData::Species.get(pkmn_data[:species]).species
          if $game_switches[61] # Random Trainer Pokemon
            pool = []
          ...
          if pkmn_data[:moves] && pkmn_data[:moves].length > 0 && !$game_switches[117] # Random Moves/Abilities
            pkmn_data[:moves].each { |move| pkmn.learn_move(move) }
          else
            pkmn.trainer_reset_moves
          end
          if pkmn_data[:ability_index] || pkmn_data[:ability]
            pkmn.ability_index = pkmn_data[:ability_index]
            pkmn.ability = pkmn_data[:ability]
          else
            pkmn.ability_index = rand(4)
            pkmn.ability = pkmn_data[:ability]
          end
          if $game_switches[61] # Random Trainer Pokemon
            ...
            if ![:ASONE,:ABILITY].include?(ability_id) # Keep original
              pkmn.ability=GameData::Ability.get(ability_id)
            end
          end
          ...
          GameData::Stat.each_main do |s|
            if pkmn_data[:iv]
              ...
              pkmn.iv[s.id] = rand(32) #[pkmn_data[:level] / 2, Pokemon::IV_STAT_LIMIT].min
            end
            if pkmn_data[:ev]
