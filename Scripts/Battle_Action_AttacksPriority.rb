class PokeBattle_Battle
    ...
    def pbRegisterMove(idxBattler,idxMove,showMessages=true)
      ...
      @choices[idxBattler][3] = -1         # No target chosen yet
      # Document player's choice in moveChoice
      if battler.pbOwnedByPlayer?
        echo("\n[AI - #{Time.now - $time}] Registering player's move choice.") if $game_switches[AIPreferences::AI_LOG_SWITCH]
        @battleAI.user = battler # Needed for field calculation
        callMove = @battleAI.changeMove(move,battler)
        battler.moveChoice = { :callMove => callMove, :realMove => move, :targets => nil,
                               :damages => nil, :moveIndex => idxMove, :targetIndex => nil,
                               :user => battler, :score => nil }
        @battleAI.user = nil
      end
      return true
    end
    ...
    def pbCalculatePriority(fullCalc=false,indexArray=nil)
      ...
      if fullCalc
        @priorityTrickRoom = @field.effects[PBEffects::TrickRoom]>0 && !($fefieldeffect == 5 &&
                             $fecounter % 6 == 3) || @field.effects[PBEffects::TrickRoom] == 0 && 
        ...
        for i in 0..maxBattlerIndex
          ...
          if @choices[b.index][0]==:UseMove || @choices[b.index][0]==:Shift
            ...
            if @choices[b.index][0]==:UseMove
              ...
              if b.hasActiveAbility?(:PRANKSTER) && move.statusMove? && $fefieldeffect != 29
                b.effects[PBEffects::Prankster] = true
              ...
              pri += 1 if b.hasActiveAbility?(:GALEWINGS) && (b.hp == b.totalhp ||
              ...
              pri += 3 if b.hasActiveAbility?(:TRIAGE) && move.healingMove? && ![10,11].include?($fefieldeffect)
              pri += 1 if b.hasActiveAbility?(:SPEEDLAUNCHER) && move.pulseMove?
              pri += 1 if b.hasActiveAbility?(:SPEEDSLICE) && move.slashingMove?
              pri += 1 if b.hasActiveAbility?(:ALPHABETIZATION) && b.checkAlphabetizationForm(16)
              pri += 1 if b.hasActiveAbility?(:ARMCANNON) && $fefieldeffect == 45 &&
                          move.punchingMove?
              pri = -1 if b.hasActiveAbility?(:MARACAMOVEMENT) && move.danceMove? &&
                          ![6,22].include?($fefieldeffect)
            ...
            subPri = 0
            # Abilities (Stall)
            #newSubPri = BattleHandlers.triggerPriorityBracketChangeAbility(b.ability,b,subPri,self)
            newSubPri = 0
            if b.hasActiveAbility?(:STALL)
              newSubPri = -1 if subPri == 0 && $fefieldeffect != 1
              newSubPri = 1 if $fefieldeffect == 45 && subPri < 1
            end
            newSubPri = 1 if b.hasActiveAbility?(:QUICKDRAW) && subPri<1 && (pbRandom(10)<3 || 
                             $fefieldeffect == 5)
            newSubPri = 1 if b.hasActiveAbility?(:QUEENLYMAJESTY) && subPri<1 && ($fefieldeffect == 5 || 
                             $fefieldeffect == 33 && $fecounter == 4)
            newSubPri = 1 if b.hasActiveAbility?(:VANGUARD) && subPri<1 && ($fefieldeffect == 5 && 
                             $fecounter%6 == 0) # Pawn
            if subPri < newSubPri#subPri!=newSubPri
              subPri = newSubPri
