module BattleHandlers
    ...
    def self.triggerMoveImmunityTargetAbility(ability,user,target,move,types,battle)
      ret = MoveImmunityTargetAbility.trigger(ability,user,target,move,types,battle)
      return (ret!=nil) ? ret : false
    end
  
    #=============================================================================
  
    def self.triggerMoveBaseTypeModifierAbility(ability,user,move,types)
      ret = MoveBaseTypeModifierAbility.trigger(ability,user,move,types)
      return (ret!=nil) ? ret : types
    end
  
    #=============================================================================
  
    def self.triggerAccuracyCalcUserAbility(ability,mods,user,target,move,types)
      AccuracyCalcUserAbility.trigger(ability,mods,user,target,move,types)
    end
  
    def self.triggerAccuracyCalcUserAllyAbility(ability,mods,user,target,move,types)
      AccuracyCalcUserAllyAbility.trigger(ability,mods,user,target,move,types)
    end
  
    def self.triggerAccuracyCalcTargetAbility(ability,mods,user,target,move,types)
      AccuracyCalcTargetAbility.trigger(ability,mods,user,target,move,types)
    end
  
    def self.triggerAccuracyCalcUserItem(item,mods,user,target,move,types)
      AccuracyCalcUserItem.trigger(item,mods,user,target,move,types)
    end
  
    def self.triggerAccuracyCalcTargetItem(item,mods,user,target,move,types)
      AccuracyCalcTargetItem.trigger(item,mods,user,target,move,types)
    end
  
    #=============================================================================
  
    def self.triggerDamageCalcUserAbility(ability,user,target,move,mults,baseDmg,types)
      DamageCalcUserAbility.trigger(ability,user,target,move,mults,baseDmg,types)
    end
  
    def self.triggerDamageCalcUserAllyAbility(ability,user,target,move,mults,baseDmg,types)
      DamageCalcUserAllyAbility.trigger(ability,user,target,move,mults,baseDmg,types)
    end
  
    def self.triggerDamageCalcTargetAbility(ability,user,target,move,mults,baseDmg,types)
      DamageCalcTargetAbility.trigger(ability,user,target,move,mults,baseDmg,types)
    end
  
    def self.triggerDamageCalcTargetAbilityNonIgnorable(ability,user,target,move,mults,baseDmg,types)
      DamageCalcTargetAbilityNonIgnorable.trigger(ability,user,target,move,mults,baseDmg,types)
    end
  
    def self.triggerDamageCalcTargetAllyAbility(ability,user,target,move,mults,baseDmg,types)
      DamageCalcTargetAllyAbility.trigger(ability,user,target,move,mults,baseDmg,types)
    end
  
    def self.triggerDamageCalcUserItem(item,user,target,move,mults,baseDmg,types)
      DamageCalcUserItem.trigger(item,user,target,move,mults,baseDmg,types)
    end
  
    def self.triggerDamageCalcTargetItem(item,user,target,move,mults,baseDmg,types)
      DamageCalcTargetItem.trigger(item,user,target,move,mults,baseDmg,types)
    end
  ...
  def pbBattleConfusionBerry(battler,battle,item,forced,flavor,confuseMsg)
    ...
    fraction_to_heal = 2
    fraction_to_heal /= 2 if battler.hasActiveAbility?(:RIPEN) && ![4,10,11,12,38,39,41,46].include?($fefieldeffect) ||
                             battler.hasActiveAbility?(:GLUTTONY) && ([15,47].include?($fefieldeffect) || 
  ...
  def pbBattleStatIncreasingBerry(battler,battle,item,forced,stat,increment=1)
    ...
    if battler.hasActiveAbility?(:RIPEN) && ![4,10,11,12,38,39,41,46].include?($fefieldeffect)
      if $fefieldeffect == 33 && $fefieldeffect == 4
        increment *= 4
  ...
  def pbBattleMoveImmunityStatAbility(user,target,move,moveTypes,immuneType,stat,increment,battle)
    return false if user.index==target.index
    return false if !moveTypes.include?(immuneType)
  ...
  def pbBattleMoveImmunityHealAbility(user,target,move,moveTypes,immuneType,battle)
    return false if user.index==target.index
    return false if !moveTypes.include?(immuneType)
    battle.pbShowAbilitySplash(target)
  ...
  def pbBattleGem(user,type,move,mults,moveTypes)
    # Pledge moves never consume Gems
    return if move.is_a?(PokeBattle_PledgeMove)
    return if !moveTypes.include?(type)
  ...
  def pbBattleTypeWeakingBerry(type,moveTypes,target,mults)
    return if !moveTypes.include?(type)
    return if !Effectiveness.super_effective?(target.damageState.typeMod) && !(target.hasActiveItem?(:CHILANBERRY) &&
              moveTypes.include?(:NORMAL))
