...
def pbBoxesFull?
  ...
  def pbStorePokemon(pkmn)
    ...
    if $Trainer.party_full?
      ...
      if swap
        $Trainer.party[$Trainer.party.length] = pkmn2
        pbMessage(_INTL("\\me[Pkmn get]{2} has added to {1}'s party!",$Trainer.name,pkmn2.name))
      end
    else
  ...
  #===============================================================================
  def pbAddPokemon(pkmn,level=1,see_form=true,ivs=[],ability=-1,moves=[],gender=0,
                   obtainText="",name="",ot="",shiny=false,evs=[],nature=-1,holditem=-1,form=-1)
    ...
    if $game_switches[60] # Random Wild Pokemon
      level = pkmn.level if pkmn.is_a?(Pokemon)
      pool = []
      GameData::Species.each { |s| pool.push(s.id) if s.form == 0 }
      pkmn = pool[rand(pool.length)]
    end
    pkmn = Pokemon.new(pkmn, level) if !pkmn.is_a?(Pokemon)
    if $game_switches[60]
      pkmn.setRandomForm(false)
    end
    if $game_switches[Settings::SHINY_WILD_POKEMON_SWITCH] || rand(65535)<Settings::SHINY_POKEMON_CHANCE || shiny
      pkmn.makeShiny
    end
    if ivs.length==6
      pkmn.iv = ivs
    ...
    if obtainText!=""
      pkmn.obtainText = _INTL(obtainText)
    end
    if name!=""
      pkmn.name = name
    end
  ...
  def pbAddRandomTestPokemonSilent
    pool = []
    GameData::Species.each { |s| pool.push(s.id) if s.form == 0 }
    species = pool[rand(pool.length)]
    3.times do
      break if GameData::Species.get(species).get_evolutions.length == 0
      newSpecies = pool[rand(pool.length)]
      species = newSpecies
    end
    mon = Pokemon.new(GameData::Species.get(species),100,$Trainer,false)
    ...
    mon.trainer_reset_moves(true,true)
