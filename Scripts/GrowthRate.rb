module GameData
    class GrowthRate
      ...
      def maximum_exp(maxLevel=GrowthRate.max_level)
        return minimum_exp_for_level(maxLevel)
      end
      ...
      def add_exp(exp1,exp2)
        #return (exp1 + exp2).clamp(0,maximum_exp(GrowthRate.max_level))
        return (exp1 + exp2).clamp(0,maximum_exp(Settings::LEVEL_LIMITS[$Trainer.badge_count]))
      end
