class PokeBattle_Battler
    ...
    def pbCanChooseMove?(move,commandPhase,showMessages=true,specialUsage=false)
      ...
      # Stuff Cheeks
      if move.function == "183" && (self.item && !GameData::Item.get(self.item).is_berry?) && 
         $fefieldeffect != 15
        if showMessages
      ...
      # Throat Chop
      if @effects[PBEffects::ThroatChop]>0 && move.soundMove?(self)
        if showMessages
      ...
      if @effects[PBEffects::ChoiceBand]
        if hasActiveItem?([:CHOICEBAND,:CHOICESPECS,:CHOICESCARF,:CHOICEDUMPLING]) &&
      ...
      if defined?(Settings::ZUD_COMPAT)
        _ZUD_Imprison(move,commandPhase)
      else
        # Imprison
  ...
  #    badgeLevel = GameData::GrowthRate.max_level if @battle.pbPlayer.badge_count >= 8
      badgeLevel = Settings::LEVEL_LIMITS[$Trainer.badge_count]
    ...
    def pbTryUseMove(choice,move,specialUsage,skipAccuracyCheck)
      ...
      when :FROZEN
        if !move.thawsUser?
          thawThreshold = ($fefieldeffect == 39) ? 10 : 20
          if @battle.pbRandom(100)<thawThreshold && $fefieldeffect != 24
            pbCureStatus
          else
            pbContinueStatus
            @lastMoveFailed = true
      ...
      if $fefieldeffect == 24 && @effects[PBEffects::Trapping] > 0
        return false
      end
      # Obedience check
      ...
      if @effects[PBEffects::Flinch]
        ...
        if hasActiveAbility?(:STEADFAST) && ![39,43,46].include?($fefieldeffect)
          if $fefieldeffect == 14
            pbRaiseStatStageByAbility(:SPEED,2,self)
          else
      ...
      if @effects[PBEffects::Confusion]>0
        ...
        if @effects[PBEffects::Confusion]<=0
          ...
          threshold = 33 # % chance
          if [24,37].include?($fefieldeffect)
            threshold = 50
          end
    ...
    def pbSuccessCheckAgainstTarget(move,user,target)
      typeMod = move.pbCalcTypeMod(move.calcTypes,user,target)
      target.damageState.typeMod = typeMod
      # Two-turn attacks can't fail here in the charging turn
      return true if user.effects[PBEffects::TwoTurnAttack]
      # Move-specific failures
      return false if move.pbFailsAgainstTarget?(user,target)
      # Field Move Fail (based on selected target)
      if move.failsDueToFieldTarget?(user,target,true)
        return false
      end
      if move.canProtectAgainst?(user)
        # Crafty Shield
        ...
        if move.pbTarget(user).num_targets > 1 && move.function != "17C"
          # Wide Guard
          ...
          if shackler && move.pbDamagingMove?
            @battle.pbCommonAnimation("WideGuard",target)
            @battle.pbDisplay(_INTL("{1}'s {2} protected {3}!",shackler.pbThis,shackler.abilityName,target.pbThis(true)))
            target.damageState.protected = true
            @battle.successStates[user.index].protected = true
            return false
          end
        ...
        # Obstruct
        if target.effects[PBEffects::Obstruct] && (move.damagingMove? || [5,14,15].include?($fefieldeffect))
        ...
        # King's Shield
        if target.effects[PBEffects::KingsShield] && (move.damagingMove? || $fefieldeffect == 31 ||
           $fefieldeffect == 5 && $fecounter%6 == 1)
          ...
          if move.pbContactMove?(user) && user.affectedByContactEffect?
            if $fefieldeffect == 31
              user.pbLowerStatStage([:ATTACK,:SPECIAL_ATTACK],2,nil)
            else
        ...
        if target.effects[PBEffects::BanefulBunker]
          ...
          @battle.successStates[user.index].protected = true
          if move.pbContactMove?(user) && user.affectedByContactEffect?
            user.pbPoison(target,nil,[19,22].include?($fefieldeffect)) if user.pbCanPoison?(target,false)
          end
          return false
        end
        # Mat Block
      ...
      if (move.canMagicCoat? || $fefieldeffect == 9 && move.numTargets(user) == 1 && move.specialMove?) && 
         !target.semiInvulnerable? && target.opposes?(user)
      ...
      if Settings::MECHANICS_GENERATION >= 7 && user.effects[PBEffects::Prankster] &&
         $fefieldeffect != 29 && (target.pbHasType?(:DARK) || [18,42].include?($fefieldeffect)) && 
         target.opposes?(user)
      ...
      if move.damagingMove? && move.calcTypes.include?(:GROUND) && $fefieldeffect != 23 &&
    ...
    def pbSuccessCheckPerHit(move,user,target,skipAccuracyCheck)
      ...
      return true if user.effects[PBEffects::TwoTurnAttack]
      # Lock-On
      return true if user.effects[PBEffects::LockOn]>0 && user.effects[PBEffects::LockOnPos]==target.index
      # Toxic
      return true if move.pbOverrideSuccessCheckPerHit(user,target)
      return true if user.effects[PBEffects::NeverMiss]
      return false if target.index == user.effects[PBEffects::AlwaysMiss]
      miss = false; hitsInvul = false
      # No Guard
      hitsInvul = true if (user.hasActiveAbility?(:NOGUARD) || target.hasActiveAbility?(:NOGUARD)) && 
                          $fefieldeffect != 14
      # Focus Moves
      hitsInvul = true if $fefieldeffect == 20 && move.focusMove?
      # Future Sight
      hitsInvul = true if @battle.futureSight
      # Helping Hand
      hitsInvul = true if move.function=="09C"
      # Alphabetization
      hitsInvul = true if user.hasActiveAbility?(:ALPHABETIZATION) && user.checkAlphabetizationForm(5)
      # Homing Cannon
      hitsInvul = true if user.hasActiveAbility?(:HOMINGCANNON) && move.beamMove?
      if !hitsInvul
