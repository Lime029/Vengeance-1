class PokeBattle_AI
    ...
    #=============================================================================
    # Registers all the scores for all the items the trainer can use on its Pokemon
    def pbRegisterItem(itemUsePartyIndex)
      # No item usage taken into account for now
    end
  ...
  =begin
    def pbEnemyShouldUseItem?
      ...
      @battle.pbRegisterItem(@attacker.index,item,idxTarget)
      PBDebug.log("[AI] #{@attacker.pbThis} (#{@attacker.index}) will use item #{GameData::Item.get(item).name}")
      return true
    end
  =end
    
    def pbEnemyShouldUseItem?
      # @user not accurate during this stage in selection
      # Account for item choice with "@battle.pbRegisterItem(@attacker.index,item,idxTarget)"
      return !@user.itemChoice.nil?
    end
  
    # NOTE: The AI will only consider using an item on the Pokémon it's currently
    #       choosing an action for.
    def pbEnemyItemToUse # Not used or updated
      return nil if !@battle.internalBattle
      items = @battle.pbGetOwnerItems(@attacker.index)
      return nil if !items || items.length==0
      # Determine target of item (always the Pokémon choosing the action)
      idxTarget = @attacker.index   # Battler using the item
      battler = @attacker
      ...
      allStatusItems = [
         ...
         :SHALOURSABLE, :BIGMALASADA, :LUMBERRY, :HEALPOWDER, :PEWTERCRUNCHIES
      ]
