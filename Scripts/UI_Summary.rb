...
class MoveSelectionSprite < SpriteWrapper
  ...
  class PokemonSummary_Scene
    ...
    def drawPageFour
      ...
      textpos = [
         [_INTL("HP"),292,70,2,base,statshadows[:HP]],
         [sprintf("%d/%d",ivs[:HP],@pokemon.ev[:HP]),462,70,1,Color.new(64,64,64),Color.new(176,176,176)],
         [_INTL("Attack"),248,114,0,base,statshadows[:ATTACK]],
         [sprintf("%d/%d",ivs[:ATTACK],@pokemon.ev[:ATTACK]),456,114,1,Color.new(64,64,64),Color.new(176,176,176)],
         [_INTL("Defense"),248,146,0,base,statshadows[:DEFENSE]],
         [sprintf("%d/%d",ivs[:DEFENSE],@pokemon.ev[:DEFENSE]),456,146,1,Color.new(64,64,64),Color.new(176,176,176)],
         [_INTL("Sp. Atk"),248,178,0,base,statshadows[:SPECIAL_ATTACK]],
         [sprintf("%d/%d",ivs[:SPECIAL_ATTACK],@pokemon.ev[:SPECIAL_ATTACK]),456,178,1,Color.new(64,64,64),Color.new(176,176,176)],
         [_INTL("Sp. Def"),248,210,0,base,statshadows[:SPECIAL_DEFENSE]],
         [sprintf("%d/%d",ivs[:SPECIAL_DEFENSE],@pokemon.ev[:SPECIAL_DEFENSE]),456,210,1,Color.new(64,64,64),Color.new(176,176,176)],
         [_INTL("Speed"),248,242,0,base,statshadows[:SPEED]],
         [sprintf("%d/%d",ivs[:SPEED],@pokemon.ev[:SPEED]),456,242,1,Color.new(64,64,64),Color.new(176,176,176)],
         [_INTL("Ability"),224,278,0,base,shadow]
      ]
      # Draw ability name and description
      ability = @pokemon.ability
    ...
    def drawSelectedMove(move_to_learn, selected_move)
      ...
      if selected_move.accuracy == 0
        ...
        textpos.push(["#{selected_move.accuracy}", 216, 180, 1, base, shadow])
      end
      # Draw all text
      pbDrawTextPositions(overlay, textpos)
