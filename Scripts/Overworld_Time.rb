...
def pbGetTimeNow
  ...
  module PBDayNight
    ...
    # Returns the cream flavor for Alcremie based on the time of day
    def self.getAlcremieCream(time=nil)
      time = pbGetTimeNow if !time
      if time.hour>=1 && time.hour<4
        cream = 2 # Matcha Cream
      elsif time.hour>=4 && time.hour<7
        cream = 7 # Caramel Swirl
      elsif time.hour>=7 && time.hour<11
        cream = 0 # Vanilla Cream
      elsif time.hour>=11 && time.hour<13
        cream = 4 # Lemon Cream
      elsif time.hour>=13 && time.hour<15
        cream = 8 # Rainbow Swirl
      elsif time.hour>=15 && time.hour<18
        cream = 5 # Salted Cream
      elsif time.hour>=18 && time.hour<20
        cream = 6 # Ruby Swirl
      elsif time.hour>=20 && time.hour<23
        cream = 1 # Ruby Cream
      elsif time.hour>=23 || time.hour<1
        cream = 3 # Mint Cream
      end
      return cream
    end
    
    # Returns the form for Lycanroc based on the time of day
    def self.getLycanrocForm(time=nil)
      time = pbGetTimeNow if !time
      if isDusk?(time)
        return 2 # Dusk
      elsif isDay?(time)
    ...
    # Returns whether the weather is clear
    def self.noWeather?
      return GameData::Weather.get($game_screen.weather_type).category == :None
    end
    
    # Returns whether the weather is sunny
    def self.sunnyWeather?
      return GameData::Weather.get($game_screen.weather_type).category == :Sun
    end
    
    # Returns whether the weather is rainy
    def self.rainyWeather?
      return GameData::Weather.get($game_screen.weather_type).category == :Rain
    end
    
    # Returns whether the weather is hail
    def self.hailWeather?
      return GameData::Weather.get($game_screen.weather_type).category == :Hail
    end
    
    # Returns whether the weather is sandstorm
    def self.sandWeather?
      return GameData::Weather.get($game_screen.weather_type).category == :Sandstorm
    end
    
    # Returns whether the weather is foggy
    def self.foggyWeather?
      return GameData::Weather.get($game_screen.weather_type).category == :Fog
    end
