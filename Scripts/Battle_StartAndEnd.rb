class PokeBattle_Battle
    ...
    def pbStartBattleSendOut(sendOuts)
      ...
      if wildBattle?
        ...
        when 1
          ...
          pbDisplayPaused(_INTL("The Pokémon is behaving strangely.\nPerhaps an external signal is interfering...")) if $game_switches[72]
        when 2
          pbDisplayPaused(_INTL("Oh! A wild {1} and {2} appeared!",foeParty[0].name,foeParty[1].name))
          pbDisplayPaused(_INTL("The Pokémon are behaving strangely. \nPerhaps an external signal is interfering...")) if $game_switches[72]
        when 3
    ...
    def pbStartBattleCore
      ...
      # Field Effect announcement
      case $fefieldeffect
      when 1
        pbDisplay(_INTL("Electricity runs across the battlefield!"))
      when 2
        pbDisplay(_INTL("Grass grows over the battlefield!"))
      when 3
        pbDisplay(_INTL("Mist swirls about the battlefield!"))
      when 4
        pbDisplay(_INTL("The crystals in the cave glimmer faintly."))
      when 5
        pbDisplay(_INTL("An army of chess pieces lines each side of the field."))
      when 6
        pbDisplay(_INTL("Let the show begin!"))
      when 7
        pbDisplay(_INTL("Magma flows throughout the battlefield."))
      when 8
        pbDisplay(_INTL("Beware the swamp creature!"))
      when 9
        pbDisplay(_INTL("A colorful prism surrounds the field."))
      when 10
        pbDisplay(_INTL("The field corrodes all that it touches!"))
      when 11
        pbDisplay(_INTL("Corrosive mist engulfs the battlefield!"))
      when 12
        pbDisplay(_INTL("The desert is hot and dry."))
      when 13
        pbDisplay(_INTL("The cave is covered in ice."))
      when 14
        pbDisplay(_INTL("Jagged rocks cover the field."))
      when 15
        pbDisplay(_INTL("Trees surround the battlefield."))
      when 16
        pbDisplay(_INTL("The volcano gurgles with magma."))
      when 17
        pbDisplay(_INTL("The roaring of machines fills your ears."))
      when 18
        pbDisplay(_INTL("The power flickers in the background..."))
      when 19
        pbDisplay(_INTL("The waste moves beneath your feet..."))
      when 20
        pbDisplay(_INTL("Ash swirls about the beach."))
      when 21
        pbDisplay(_INTL("The water is a deep, deep blue."))
      when 22
        pbDisplay(_INTL("Light can be seen faintly overhead at the water's surface..."))
      when 23
        pbDisplay(_INTL("The cave is dim and musty."))
      when 24
        pbDisplay(_INTL("Error! The dat4 h@s b3E\\| c0R&#|3|)"))
      when 25
        pbDisplay(_INTL("Crystals sparkle brightly in the background."))
      when 26
        pbDisplay(_INTL("Murk swirls about the water..."))
      when 27
        pbDisplay(_INTL("A beautiful landscape reveals itself below."))
      when 28
        pbDisplay(_INTL("The mountain is covered in snow."))
      when 29
        pbDisplay(_INTL("Lord, answer my prayers!"))
      when 30
        pbDisplay(_INTL("Mirror, mirror, on the wall,"))
        pbDisplay(_INTL("Who's the strongest of them all?"))
      when 31
        pbDisplay(_INTL("A land where every fantasy becomes reality..."))
      when 32
        pbDisplay(_INTL("A beastly roar echoes from deep within the cave..."))
      when 33
        pbDisplay(_INTL("The ground is rich and fertile."))
      when 34
        pbDisplay(_INTL("When you wish upon a star,"))
        pbDisplay(_INTL("Makes no difference who you are."))
        pbDisplay(_INTL("Anything your heart desires will come to you..."))
      when 35
        pbDisplay(_INTL("Cries from distant worlds reverberate..."))
      when 36
        pbDisplay(_INTL("!trats elttaB"))
      when 37
        pbDisplay(_INTL("Mysterious energy spreads throughout the battlefield!"))
      when 38
        pbDisplay(_INTL("Dark energy engulfs the field."))
      when 39
        pbDisplay(_INTL("When hell freezes over..."))
      when 40
        pbDisplay(_INTL("Evil spirits lurk in the background..."))
      when 41
        pbDisplay(_INTL("Toxic chemicals flow throughout the cave."))
      when 42
        pbDisplay(_INTL("Eyes can be seen watching closely from the trees..."))
      when 43
        pbDisplay(_INTL("The clear sky is visible above the clouds."))
      when 44
        pbDisplay(_INTL("It's quiet and tidy inside."))
      when 45
        pbDisplay(_INTL("Ready, fight!"))
      when 46
        pbDisplay(_INTL("A slick coat of ice and snow spans as far as the eye can see."))
      when 47
        pbDisplay(_INTL("In the jungle, the mighty jungle..."))
      when 48
        pbDisplay(_INTL("An ocean breeze puts the mind at ease."))
      when 49
        pbDisplay(_INTL("The desert is littered with shrubs."))
      end
  =begin
  ...
  =end
    ...
    def pbLoseMoney
      return if !@internalBattle || !@moneyGain || !trainerBattle?
      return if $game_switches[Settings::NO_MONEY_LOSS] || $game_switches[104] # Celebi Rescue
      tMoney = 0
      @opponent.each_with_index do |t,i|
        tMoney += pbMaxLevelInTeam(1, i) * t.base_money # Lose as much money as you would've gained
      end
    ...
    def pbEndOfBattle
      ...
      $game_variables[39] = $fefieldeffect
      $game_switches[108] = false # Set Style Override
