...
ItemHandlers::CanUseInBattle.add(:GUARDSPEC,proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
  ...
  ItemHandlers::CanUseInBattle.copy(:POTION,
     ...
     :ENERGYROOT,:ULTRAPOTION,:BRITTLEBONES,:VANILLAICECREAM,:CHOCOLATEICECREAM,
     :STRAWBERRYICECREAM,:BLUEMOONICECREAM,:BOILEDEGG,:BREAD,:INSTANTNOODLES,
     :FRIEDFOOD,:KRABBYCAKE,:COCONUTMILK,:MOOMOOCHEESE,:PRECOOKEDBURGER,
     :PACKAGEDCURRY,:PASTA,:SMOKEPOKETAIL,:PEKINGPSYDUCK,:SHARPEDOFINSOUP,
     :GIGANTAMIX)
  ...
  ItemHandlers::CanUseInBattle.copy(:FULLHEAL,
     ...
     :BIGMALASADA,:LUMBERRY,:HEALPOWDER,:PEWTERCRUNCHIES)
  ...
  ItemHandlers::CanUseInBattle.add(:COFFEE,proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
    if !pokemon.able? || pokemon.hp == pokemon.totalhp && pokemon.status != :SLEEP
      scene.pbDisplay(_INTL("It won't have any effect.")) if showMessages
      next false
    end
    next true
  ...
  ItemHandlers::CanUseInBattle.add(:KONIKONIFRIEDCOMBUSKEN,proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
    if !pokemon.able? || pokemon.hp == pokemon.totalhp && pokemon.status != :FROZEN
      scene.pbDisplay(_INTL("It won't have any effect.")) if showMessages
      next false
    end
    next true
  ...
  ItemHandlers::CanUseInBattle.add(:FRESHCREAM,proc { |item,pokemon,battler,move,firstAction,battle,scene,showMessages|
    if !pokemon.able? || pokemon.hp == pokemon.totalhp && pokemon.status != :BURN
      scene.pbDisplay(_INTL("It won't have any effect.")) if showMessages
      next false
    end
    next true
  ...
  ItemHandlers::BattleUseOnPokemon.copy(:POTION,:BERRYJUICE)
  ...
  ItemHandlers::BattleUseOnPokemon.copy(:SUPERPOTION,:BREAD)
  ...
  ItemHandlers::BattleUseOnPokemon.add(:ULTRAPOTION,proc { |item,pokemon,battler,choices,scene|
  ...
  ItemHandlers::BattleUseOnPokemon.copy(:ORANBERRY,:BRITTLEBONES)
  ...
  ItemHandlers::BattleUseOnPokemon.add(:KONIKONIFRIEDCOMBUSKEN,proc { |item,pokemon,battler,choices,scene|
    if pokemon.status == :FROZEN
      pokemon.heal_status
      battler.pbCureStatus(false) if battler
      name = (battler) ? battler.pbThis : pokemon.name
      scene.pbRefresh
      scene.pbDisplay(_INTL("{1} thawed out.",name))
    end
    pbBattleHPItem(pokemon,battler,50,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:FRESHCREAM,proc { |item,pokemon,battler,choices,scene|
  ...
  ItemHandlers::BattleUseOnPokemon.add(:SWEETHEART,proc { |item,pokemon,battler,choices,scene|
    if pbBattleHPItem(pokemon,battler,20,scene)
      pokemon.changeHappiness("regicecream")
    end
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:VANILLAICECREAM,proc { |item,pokemon,battler,choices,scene|
    if pbBattleHPItem(pokemon,battler,30,scene)
      pokemon.changeHappiness("regicecream")
    end
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:CHOCOLATEICECREAM,proc { |item,pokemon,battler,choices,scene|
    if pbBattleHPItem(pokemon,battler,70,scene)
      pokemon.changeHappiness("regicecream")
    end
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:STRAWBERRYICECREAM,proc { |item,pokemon,battler,choices,scene|
    if pbBattleHPItem(pokemon,battler,90,scene)
      pokemon.changeHappiness("regicecream")
    end
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:BLUEMOONICECREAM,proc { |item,pokemon,battler,choices,scene|
    if pbBattleHPItem(pokemon,battler,200,scene)
      pokemon.changeHappiness("bluemoon")
    end
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:BOILEDEGG,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,40,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:INSTANTNOODLES,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,80,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:FRIEDFOOD,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,90,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:KRABBYCAKE,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,110,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:COCONUTMILK,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,120,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:MOOMOOCHEESE,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,130,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:PRECOOKEDBURGER,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,140,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:PACKAGEDCURRY,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,150,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:PASTA,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,160,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:SMOKEPOKETAIL,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,170,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:PEKINGPSYDUCK,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,180,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:SHARPEDOFINSOUP,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,190,scene)
  })
  
  ItemHandlers::BattleUseOnPokemon.add(:GIGANTAMIX,proc { |item,pokemon,battler,choices,scene|
    pbBattleHPItem(pokemon,battler,200,scene)
  })
